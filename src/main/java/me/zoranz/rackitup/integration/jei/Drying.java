...
import me.zoranz.rackitup.tag.MTags;
...
public class Drying implements IRecipeCategory<DryingRecipe> {
    ...
    public void draw(DryingRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
        ...
        if(Config.CONFIG.boosterCampfires.get()){
            double boost = Math.max(1, Config.CONFIG.campfireBoost.get());
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, DryingRecipe recipe, IFocusGroup iFocusGroup) {
        ...
        if(boosting){
            slot.addRichTooltipCallback((view, tooltip) -> {tooltip.add(Component.translatable("jei.rackitup.drying_speed_mult", Config.CONFIG.campfireBoost.get()).withStyle(ChatFormatting.GOLD));});
        }
